...
pub async fn create_transaction_pay_frame(configuration: &configuration::Configuration, frame_pay_transaction_request_body: models::FramePayTransactionRequestBody) -> Result<models::TransactionFrameResponse, Error<CreateTransactionPayFrameError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_frame_pay_transaction_request_body = frame_pay_transaction_request_body;
    ...
    req_builder = req_builder.json(&p_frame_pay_transaction_request_body);
...
pub async fn fetch_user_interactions(configuration: &configuration::Configuration, fids: &str, r#type: Option<Vec<models::NotificationType>>) -> Result<models::FetchUserInteractions200Response, Error<FetchUserInteractionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_fids = fids;
    let p_type = r#type;
...
pub async fn lookup_cast_conversation_summary(configuration: &configuration::Configuration, identifier: &str, limit: Option<i32>, prompt: Option<&str>) -> Result<models::ConversationSummary, Error<LookupCastConversationSummaryError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_identifier = identifier;
    let p_limit = limit;
    let p_prompt = prompt;

    let uri_str = format!("{}/farcaster/cast/conversation/summary", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<LookupCastConversationSummaryError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
