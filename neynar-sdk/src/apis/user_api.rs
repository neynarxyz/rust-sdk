...
pub async fn fetch_bulk_users_by_eth_or_sol_address(configuration: &configuration::Configuration, addresses: &str, address_types: Option<Vec<models::BulkUserAddressType>>, viewer_fid: Option<i32>, x_neynar_experimental: Option<bool>) -> Result<std::collections::HashMap<String, Vec<models::User>>, Error<FetchBulkUsersByEthOrSolAddressError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `std::collections::HashMap&lt;String, Vec&lt;models::User&gt;&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `std::collections::HashMap&lt;String, Vec&lt;models::User&gt;&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
