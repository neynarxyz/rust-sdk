...
pub struct FetchRelevantFungibleOwnersParams {
    ...
    pub network: models::FungibleOwnerRelevantNetwork,
...
/// struct for passing parameters to the method [`send_fungibles_to_users`]
#[derive(Clone, Debug)]
pub struct SendFungiblesToUsersParams {
    /// Wallet ID to use for transactions
    pub x_wallet_id: String,
    pub transaction_send_fungibles_request: models::TransactionSendFungiblesRequest,
}
...
/// struct for typed errors of method [`send_fungibles_to_users`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SendFungiblesToUsersError {
    Status400(models::ErrorRes),
    Status500(models::ErrorRes),
    UnknownValue(serde_json::Value),
}
...
pub async fn send_fungibles_to_users(
    configuration: &configuration::Configuration,
    params: SendFungiblesToUsersParams,
) -> Result<models::TransactionSendFungiblesResponse, Error<SendFungiblesToUsersError>> {
    let uri_str = format!("{}/farcaster/fungible/send", configuration.base_path);
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = req_builder.header("x-wallet-id", params.x_wallet_id.to_string());
    if let Some(ref apikey) = configuration.api_key {
        let key = apikey.key.clone();
        let value = match apikey.prefix {
            Some(ref prefix) => format!("{} {}", prefix, key),
    ...
    req_builder = req_builder.header(reqwest::header::CONTENT_TYPE, "application/json");
    req_builder = req_builder.json(&params.transaction_send_fungibles_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TransactionSendFungiblesResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TransactionSendFungiblesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SendFungiblesToUsersError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
