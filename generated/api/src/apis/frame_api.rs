...
pub struct FetchFrameCatalogParams {
    ...
    pub time_window: Option<models::MiniAppTimeWindow>,
...
/// struct for passing parameters to the method [`get_notification_campaign_stats`]
#[derive(Clone, Debug)]
pub struct GetNotificationCampaignStatsParams {
...
/// struct for passing parameters to the method [`search_frames`]
#[derive(Clone, Debug)]
pub struct SearchFramesParams {
...
/// struct for typed errors of method [`get_notification_campaign_stats`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetNotificationCampaignStatsError {
    Status400(models::ErrorRes),
    Status500(models::ErrorRes),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`search_frames`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SearchFramesError {
    Status400(models::ErrorRes),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_notification_campaign_stats(
    configuration: &configuration::Configuration,
    params: GetNotificationCampaignStatsParams,
) -> Result<models::GetNotificationCampaignStats200Response, Error<GetNotificationCampaignStatsError>>
{
    let uri_str = format!("{}/farcaster/frame/notifications", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = params.campaign_id {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetNotificationCampaignStats200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetNotificationCampaignStats200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetNotificationCampaignStatsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn search_frames(
...
) -> Result<models::FrameCatalogResponse, Error<SearchFramesError>> {
    let uri_str = format!("{}/farcaster/frame/search", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SearchFramesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
