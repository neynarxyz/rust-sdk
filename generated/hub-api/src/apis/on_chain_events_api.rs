...
pub async fn fetch_user_on_chain_events(
    configuration: &configuration::Configuration,
    params: FetchUserOnChainEventsParams,
) -> Result<models::FetchUserOnChainEvents200Response, Error<FetchUserOnChainEventsError>> {
...
pub async fn fetch_user_on_chain_signers_events(
    configuration: &configuration::Configuration,
    params: FetchUserOnChainSignersEventsParams,
) -> Result<
    models::FetchUserOnChainSignersEvents200Response,
    Error<FetchUserOnChainSignersEventsError>,
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FetchUserOnChainSignersEventsError> =
...
pub async fn lookup_on_chain_id_registry_event_by_address(
    configuration: &configuration::Configuration,
    params: LookupOnChainIdRegistryEventByAddressParams,
) -> Result<models::OnChainEventIdRegister, Error<LookupOnChainIdRegistryEventByAddressError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<LookupOnChainIdRegistryEventByAddressError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
