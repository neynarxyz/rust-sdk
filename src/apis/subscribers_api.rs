...
pub async fn fetch_subscription_check(configuration: &configuration::Configuration, addresses: &str, contract_address: &str, chain_id: &str) -> Result<std::collections::HashMap<String, models::SubscriptionStatus>, Error<FetchSubscriptionCheckError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `std::collections::HashMap&lt;String, models::SubscriptionStatus&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `std::collections::HashMap&lt;String, models::SubscriptionStatus&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
